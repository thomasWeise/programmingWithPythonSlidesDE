...
\item<10-> Alles, was wir in anderen Quellen finden, muss also in der offiziellen Dokumentation nachgeprüft werden.%
\end{itemize}%
\end{frame}%
%
...
%
\only<-5>{\item<5-> Wir kommen auf die Seite \inQuotes{Hi，AI搜索已支持R1满血版，快来试试吧！}, auf der wir unseren Prompt für das DeepSeek\nobreakdashes-R1 Modell\cite{DAGYZSZXZMWBZYWWGSLGLXWWFLZDZRDCJLLDLHCLZBXWDXGQLGLWCYQLCNLCDHGGHYWZZWZXXZZTLWLTHZWCDGZPWCJCLZCYWYZPL2024DRIRCILVRL} in das Eingabefeld unten eintippen können%
}%
%
\only<-6>{\item<6-> Wir schreiben \inQuotes{How can I round floating point numbers towards positive infinity in Python?} und drücken~\keys{\return}.%%
}%
%
...
\only<-5,8,10->{%
%
\item Ein wichtiges Features von Software sind durchgängige, einheitliche Architektur und Stil.\uncover<2->{ Ein aus \pgls{AI}-Versatzstücken zusammengeschustertes System lässt sich nicht warten oder verstehen.}%
%
...
\item<8-> Hier ist ein Beispiel, wie eine Vibe-Coding \pgls{AI} die ganze Festplatte eines Benutzers gelöscht hat anstelle eines Ordners.%
%
\item<10-> \pgls{AI}-Werkzeuge schlagen manchmal das Benutzen von \alert{nicht existierenden} Packages vor, was zu großen Sicherheitsproblemen im Kode führen kann\cite{AT:G2025AGCCBADFTSSCHW}.%
%
\end{itemize}%
%
...
%
\locateGraphicTB[Quelle \& Copyright:~\bracketCite{S2025GAVCTLUGDT}]{9}{width=0.45\paperwidth}{graphics/aiHddDelete/aiHddDeleteRegister}{0.033333333}{0.3}%
\locateGraphicTB[Quelle \& Copyright:~\bracketCite{V2025CCGAVCUEDOOE}]{9}{width=0.45\paperwidth}{graphics/aiHddDelete/aiHddDeleteGolem}{0.516666666}{0.3}%
\end{frame}%
%
\begin{frame}%
