...
%
\only<-5>{%
\item<2-> \ruff\ sieht die Konstruktion der Liste über die \pythonilIdx{append}-Funktion in einer Schleife als \emph{Performanz-Problem} an, was durch die Kennung \textil{PERF} ausgedrückt wird.%
}%
\only<-6>{%
...
%
\only<-10>{%
\item<2-> Um diese Sache zu untersuchen, könnten wir versuchen, zwei Listen mit dem selben Inhalt zu erstellen.%
}%
%
...
%
\only<-32>{%
\item<23-> Deshalb sagen wir der \pythonilIdx{repeat}-Funktion, 50~solcher Messungen durchzuführen (Argument \pythonil{repeat=50}).%
}%
%
\only<-34>{%
...
%
\only<-37>{%
\item<33-> Die Funktion \pythonilIdx{min} akzeptiert eine Sequenz von Elementen und gibt uns das kleinste Element daraus zurück.\cite{PSF:P3D:TPSL:BIF}%
}%
%
\only<-38>{%
