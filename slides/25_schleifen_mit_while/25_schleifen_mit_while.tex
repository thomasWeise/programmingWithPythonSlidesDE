...
%
\only<-8>{%
\item Alte Lehmtablets beweisen, dass Babylonier $\sqrt{2}$ vielleicht schon vor 4000 Jahren annähern konnten\cite{FR1998SRAIOBMY7IC,S2011NA:NA}.%
}%
%
\item<2-> Der Mathematiker \emph{Hero(n) von Alexandria} hat im ersten Jahrhundert~\glsFull{CE} gelebt. %
Er hat einen Algorithmus zum Berechnen der Quadratwurzel entwickelt, den wir heute als Heron's Methode kennen\cite{S2011NA:NA,K2009BMOCTSRJBOFTAOCC}.%
%
...
\begin{itemize}%
\only<-7>{%
\item Wenn wir Heron's Methode implementieren wollen, dann brauchen wir irgendeine Art Schleife.%
%
...
%
\only<-4>{%
\item Wir benutzen nun dieses Konstrukt, um eine binäre Suche\cite{K1998SAS,H2024POIC:BS,B1999PP}~\inEN{binary search} zu implementieren.%
}%
%
\only<-5>{%
...
%
\only<-7>{%
\item<4-> In jedem Schritt wollen wir die Größe des Segments verringern, in dem wir die \emph{Hälfte}, in der $E$ nicht seien kann, ausschließen.%
}%
%
\only<-8>{%
...
%
\only<-31>{%
\item<29-> Warnung: Interessanterweise ist das so nur korrekt in \python~3 mit seinen beliebig großen Ganzzahlen. In Sprachen wir \pgls{C} or \pgls{Java}, wo Ganzzahlen feste Grenzen haben, müssen wir stattdessen \pythonil{mid = lower + (upper}\pythonil{ - lower) // 2} rechnen\cite{H2024POIC:BS}.%
}%
%
\only<-33>{%
...
\item<7-> Wir können Wurzeln mit dem Algorithmus von Heron berechnen.%
\item<8-> Wir können binäre Suche implementieren.%
\end{itemize}%
}%
