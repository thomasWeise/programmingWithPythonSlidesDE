...
Die Abfolge der Parameter-Datentypen und der Datentyp des Rückgabewertes einer Funktion zusammen werden als die \emph{Signatur}~\inEN{signature} der Funktion bezeichnet.%
\end{definition*}%
}%
%
\only<28>{%
...
%
\only<-22>{%
\item<18-> Die exklusive Obergrenze \pythonil{a + 1} der \pythonilIdx{range}, so das die \pythonil{for}-Schleife läuft, bis \pythonil{i} irgendwann \pythonil{a} erreicht.%
}%
%
\only<-23>{%
...
%
\item<38-> Nun da wir fertig sind, lassen Sie mich noch erwähnen, dass es eine besonders effiziente binäre Variante des Euklidischen Algorithmus gibt, die scheller als unsere Implementierung ist und bereits in China im ersten Jahrhundert~\glsFull{CE} entwickelt wurde\cite{B1999FAOTBEA} und zwar steht die im \emph{Jiu Zhang Suanshu}~(九章算术)\cite{OR2003LH,SCL1999TNCOTMACAC,S1998LHATFGAOCM,D2010AALHOCAS,C2002LFLHADWTDM}.%
%
\end{itemize}%
}}%
%
