...
%
\only<-11>{%
\item<8-> Im Falle der Funtkion \pythonil{sqrt} war das die Zahl, deren Wurzel berechnet werden sollte.%
}%
%
\only<-11>{%
...
\only<-17>{%
\item<14-> Jeder Parameter hat einen Namen, unter dem wir dann in der Funtktion auf seinen Wert zugreifen können.%
}%
\only<-17>{%
...
Alle Parameter und der Rückgabewert einer Funtkion sollten mit \glslink{typeHint}{Type Hints}\cite{PEP3107} annotiert werden. %
...
%
\item<2-> Funktionen können ein Ergebnis zurückliefern oder auch nichts\uncover<3->{ (wobei sie dann \pythonil{None} zurückliefern, wie wir in Einheit~12 gelernt haben)}.%
%
...
%
\item<6-> Schauen wir uns die Berechnung des größten gemeinsamen Teilers, auf Englisch \emph{greatest common divisor}, kurz~\pythonil{gcd} an.%
}%
%
\only<-16>{%
