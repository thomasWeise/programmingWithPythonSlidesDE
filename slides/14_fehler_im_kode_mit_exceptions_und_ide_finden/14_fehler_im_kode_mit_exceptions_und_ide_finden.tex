...
\begin{itemize}%
\item In den zukünftigen Slides werden wir eher weniger Screenshots vom \pycharm\ \glsShort{ide} verwenden.%
...
\item<4-> Bevor wir uns aber von den Screenshots verabschieden, wollen wir uns noch eine wichtige Funktion anschauen, die viele \glsShort{ide} bereitstellen\only<-4>{.}\uncover<5->{:}%
...
\item<11-> Manchmal müssen wir lange \glslink{debugger}{debuggen}, um die Fehler zu entdecken~(das kommt später in Einheit~\unitDebugger).%
\item<12-> Bei manchen einfachen Fällen, wie Tippfehlern, kann uns eine gute \glsShort{ide} aber schon helfen.%
\end{itemize}%
\end{frame}%
%
...
%
\only<-2>{%
\item Das Programm \programUrl{variables:assignment_wrong}, eine fehlerhafte Variante des Programmes \textil{assignment.py} aus der vorigen Einheit.%
}%
%
\only<-3>{%
...
%
\end{itemize}%
\listingPython{}{variables:assignment_wrong}{0.15}{0.33}{0.7}{0.7}%
\end{frame}%
%
\begin{frame}[t]%
...
%
\only<-3>{%
\item<2-> Wir können das Programm auch in dem \pycharm\ \glsShort{ide} ausführen, in dem wir entweder auf \pycharmRun\ klicken oder in dem wir \keys{\shift+F10} drücken.%
}%
%
\only<-6>{%
...
%
\only<-7>{%
\item<4-> Wenn ein Programm crashed, dann ist \alert{das Erste}, was wir machen, uns alle Ausgaben \alert{sehr genau} anzuschauen.%
}%
%
\only<-8>{%
...
%
\only<-15>{%
\item<12-> Mehrnoch, er teilt uns sogar die genaue Datei und Zeilennummer mit, wo der Fehler passiert ist:~in Zeile~12 von Datei~\programUrl{variables:assignment_wrong}!%
}%
%
\only<-16>{%
...
%
\item<14-> Dieser so genannte \pythonil{Exception} \glslink{stackTrace}{Stack Trace}, der auf dem \emph{Standard Error Stream (\glsShort{stderr})} ausgegeben wrd, sagt uns also nicht nur, dass es einen Fehler gab.%
 %
\item<15-> Er sagt uns auch, was der wahrscheinlichste Grund für den Fehler und wo er wahrscheinlich stattfand.%
%
\item<16-> Wir diskutieren das Thema \pythonilsIdx{Exception} später~(ab Einheit~\unitRaiseExceptions), aber bereits jetzt sollte die Sache ziemlich klar sein:%
%
\end{itemize}%
%
\listingOutput{1,4-16}{variables:assignment_wrong}{style=tool_style}{0.1}{0.56}{0.9}{0.7}%
...
%
\uncover<17->{%
\bestPractice{readErrorMessage}{%
Lesen Sie immer alle Fehlermeldungen. %
Sie geben uns oft wichtige Informationen, wenn wir nach Fehlern suchen. %
\alert{Fehlermeldungen nicht zu lesen ist falsch.}%
}}%%
%
\end{frame}%
%
...
\only<8->{%
\only<-9>{%
\item<8-> Wir kennen jetzt also schon zwei Methoden, Fehler im Kode mit Hilfe des \glsShort{ide} zu finden.%
}%
%
\only<-10>{%
...
%
\only<-11>{%
\item<10-> Die \pycharm\ \glsShort{ide} zeigt uns auch das kleine rote~\pycharmErrorsSymbol~Symbol in der oberen rechten Ecke.%
}%
%
\only<-12>{%
...
\end{itemize}%
%
\locateGraphicTB{1-2}{width=0.8\paperwidth}{graphics/errorsInIde02exception}{0.1}{0.38}%
...
Wenn wir Kode schreiben, sollten wir immer aufpassen, ob das \glsShort{ide} uns über irgendwelche mögliche Fehler benachrichtigt. %
...
Das \glsShort{ide} und die Fehlermeldungen (\pythonilIdx{Exception} \glslink{stackTrace}{Stack Traces}) sind Ihre wichtigsten Werkzeuge um Fehler zu finden. %
Lesen Sie die Fehlermeldungen. %
Gleichgültig ob Sie \pycharm\ oder irgendein anderes \glsShort{ide} verwenden, wenn dieses \glsShort{ide} Ihren Kode mit Warnungen und Fehlerhinweisen annotiert, dann lesen und prüfen Sie jeden einzelnen Hinweis.%
}%
}%
%
\end{frame}%
...
\item<3-> Wenn unser \glsShort{ide} mögliche Fehler und Warnungen anzeigt, dann schauen wir uns diese genau an.%
...
%
\item<7-> Prüfen Sie daher immer alle Hinweise Ihres \glsShort{ide}.%
%
...
\item<3-> Auf der anderen Seite können wir viele Hinweise und Warnungen bereits in dem \pycharm\ \glsShort{ide} sehen.%
