...
%
\only<-3>{%
\item Generator-Ausdrücke\cite{PEP289} existieren, damit wir \inQuotes{bequem} Sequenzen erstellen können, die ihre Elemente bei Bedarf erstellen und zurückgeben, anstalle alle im Speicher zu halten.%
}%
%
\only<-4>{%
...
%
\only<-44>{%
\item<40-> Das nennt man \emph{lazy evaluation}.%
}%
%
\only<-45>{%
...
%
\only<-12>{%
\item<8-> Wir schreiben ihn einmal hin, um ihn an \pythonilIdx{max} zu übergeben, damit wir den größten Wert bekommen können.%
}%
%
\only<-12>{%
