...
%
\item<11-> Es sind Ausnahmen (englisch:~Exceptions).
\end{itemize}%
\end{frame}%
%
\begin{frame}%
...
%
\item<11-> \alert{Wir können die Funktion beschützen, in dem wir eine Ausnahme auslösen.}\cite{MAGTOC2024EHFAP,SF2013ASROSR,GRRX2001ACSOEHMFBDOOS}~(Auf Englisch \emph{raise an \pythonil{Exception}})%
\end{enumerate}%
}%
%
...
%
\item<3-> Das Auslösen von Ausnamen, in Englisch \emph{raising of \pythonilsIdx{Exception}} bedeutet zwei Dinge\only<-3>{.}\uncover<4->{%
%
\begin{enumerate}%
%
...
\begin{itemize}%
\only<-3>{%
\item Ausnahmen auslösen, auf Englisch/in \python: \emph{\inQuotes{raising exceptions}}, ist sehr einfach.%
}%
%
\only<-4>{%
...
%
\only<-6>{%
\item<2-> Wir gehen dafür zurück zu unserer Funktion \pythonil{sqrt} in Datei \programUrl{functions:my_math_3} aus Einheit~28.%
}%
%
\only<-7>{%
...
%
\only<-15>{%
\item<12-> Wir hatten diese Informationen schonmal in Einheit~14 erwähnt, als es darum ging, Fehler mit Hilfe der \glslink{ide}{IDE} zu finden.%
}%
%
\only<-17>{%
