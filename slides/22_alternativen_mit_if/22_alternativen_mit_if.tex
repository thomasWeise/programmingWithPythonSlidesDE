...
\item<2-> Die Kondition ist ein einfacher Boolescher Ausdruck, wie wir ihn bereits in Einheit~10~(\pythonil{bool}) diskutiert haben.%
\item<3-> Die Syntax dafür ist sehr einfach:%
}%
%
\only<-8>{%
...
\only<-4>{%
\item<4-> Aus der 7.~Einheit erinnern Sie sich vielleicht noch an den Modulo-Operator~\pythonilIdx{\%}.%
}%
%
\only<-9>{%%
\item<5-> Wenn wir den Modulo-Operator auf zwei Werte \pythonil{a} und \pythonil{b} anwenden, liefert er uns den Rest der Division~\pythonil{a // b}\pythonIdx{//}.%
}%
%
\only<-10>{%
\item<6-> Wenn der Rest~0~ist, dann ist \pythonil{a} durch \pythonil{b} teilbar.%
}%
%
\only<-10>{%
...
%
\only<-9>{%
\item<5-> Natürlich geht das auch einfacher: mit \pythonil{if...else}.%
}\only<-10>{%
...
%
\only<-5>{%
\item Probieren wir das mal aus in unserem Programm \textil{if_else_nested.py}.%
}\only<-6>{%
...
%
\item<7-> Darum wurde das \pythonilIdx{elif}-Statement entwickelt, das genau ein \pythonil{else}, welches nur ein \pythonil{if} beinhaltet, ersetzt~(oder eher kombiniert).%
%
...
%
\only<-5>{%
\item Wir benutzen diese Syntax\only<-4>{ in Programm \textil{if_elif_example.py}}, um die Lebensphase einer Person basierend auf ihrem Alter zu berechnen.%
}\only<-6>{%
...
\parbox{0.4\paperwidth}{\small{%%
\begin{itemize}%
\item Schauen wir uns nochmal unser Programm \textil{if_else_nested.py} an.%
\only<-11>{%
...
%
\item<5-> Wir könnten jetzt schon hard-kodierte Entscheidungsbäume implementieren~\cite{RN2022AIAMA,SSBD2014UMLFTTA} und im Grunde war ja \textil{if_elif_example.py} ein Beispiel genau dafür.%
%
