...
%
\only<-15>{%
\item<12-> Wir benutzen Klammern um die Ausdruck einfacher lesbar zu machen.%
}%
%
\only<-16>{%
...
%
\item<6-> Können wir Alternativen verschachteln~\inEN{nest}??%
%
...
\item<10-> Diese Werkzeuge können uns schon echt helfen, die Kode-Qualität zu verbessern.%
...
%
\item<2-> In Programmiersprachen wie \pgls{C} und \pgls{Java} haben wir einen ternären Operator wie \textil{bedingung ? WertWennJa : WertWennFalsch} für solche Fälle.%
%
...
%
\item<7-> Mit der Ausnahme, dass wir Befehle jetzt auch übergehen können, werden unsere Programm allerdings immer noch in der Reihenfolge abgearbeitet, in der wir sie aufgeschrieben haben.%
%
\item<8-> Wir können Instruktionen noch nicht mehrfach ausführen bzw.\ im Kode \inQuotes{zurück springen}.%
%
\item<9-> Warten Sie mal auf die nächsten Einheiten\dots%
\end{itemize}%
%
\end{frame}%
%
