...
\item<2-> In der Tat gibt es eine Möglichkeit, schön formatierte Zeichenketten zu erstellen, die Ergebnisse von Berechnungen beinhalten können\only<-2>{.}\uncover<3->{:~\glsFullpl{fstring}.}%
\item<4-> \Glspl{fstring} erlauben es uns, die Ergebnisse von Ausdrücken in Zeichenketten einzufügen\cite{PEP498,PSF:P3D:TPT:FSL,M2017WAFSIPAHCIUT,B2023PFS,DEVC:WFAAPODSCMIP,G2025MFIP}.%
...
\item<9-> Während der sogenannten \glsFull{strinterpolation}, welche automatisch passiert, werden diese Ausdrücke dann ausgerechnet und ihre Ergebnisse an ihrer Stelle in den Text eingefügt.%
\end{enumerate}%
}%
\only<-10>{%
...
%
\only<-13>{\item<12-> Eine \pythonil{int}-Zahl ist ein Ausdruck. Schreiben wir also mal einen \gls{fstring}, der eine Zahl in einen Text umwandelt und mit anderem Text kombiniert.\uncover<13->{ Das funktioniert problemlos.%
}}%
%
\only<-15>{\item<14-> \Glspl{fstring} erlauben es uns auch, die Erbenisse von Ausdrücken zu formatieren. Schreiben wir z.B.\ \textil{:,} hinten an den Ausdruck (hier: der Zahl) ran, dann wird der Ausdruck als Zahl behandelt und es werden Tausender-Trennzeichen~(hier: Kommas) bei der Umwandlung in Text eingefügt.\uncover<15->{ Aus der Zahl \textil{12345678901234} wird der Text~\textil{12,345,678,901,234}.%%
}}%
%
...
%
\only<-27>{\item<26-> Wir können in \glspl{fstring} auf \alert{alle} Werte zugreifen, auf die wir aktuell Zugriff haben. Importieren wir also interessehalber mal wieder \numberPi\ from \pythonil{math}\nobreakdashes-Modul.\uncover<27->{ Was natürlich klappt.%
}}%
%
\only<-29>{\item<28-> Nun können wir auch \pythonil{pi} in \glspl{fstring} verwenden.\uncover<29->{ Und sein Wert wird eingefügt.%
}}%
%
...
%
\only<-39>{\item<38-> Und schon können wir sie in \glspl{fstring} verwenden, deren Ausgabe wir wieder beliebig formatieren können.\uncover<39->{ $\sin{\frac{\numberPi}{4}}=\frac{\sqrt{2}}{2}\approx0.7071067811865476$.%
}}%
%
...
%
\only<-49>{\item<48-> Kommen wir nun zu unserem Anfangsbeispiel, dem komplizierten String \pythonil{str(1 < 5) + \" is True and \" + str(1 + 5) + \" = 6.\"}, zurück. Als \gls{fstring} sieht das so aus: \pythonil{f\"\{1 < 5\} is True and \{1 + 5\} = 6.\"}\uncover<47->{ Beide Varianten ergeben \pythonil{\"True is True and 6 = 6.\"}.%
}}%
%
