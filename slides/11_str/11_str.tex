...
\item<4-> Wir haben sie bereits oftmals implizit oder explizit genutzt, \DEzB\ in unserem ersten Programm, das einfach \pythonil{\"Hello World\"} ausgedruckt hat.%
...
Somit liefert die Funktion~\pythonil{-1} zurück.\uncover<12->{ %
...
\item<3-> Wir brauchen sie dann aber oft als andere Datentypen, \DEzB\ als Zahlen.%
\item<4-> Viele Datentypen in \python\ bieten eine Funktion an, die eine Zeichenkette in sie umwandelt.%
\only<-6>{%
\item<5-> \pythonil{int(\"4\")} erzeugt \DEzB\ den \pythonil{int}\nobreakdashes-Wert~\pythonil{4}.%
}%
}%
\only<-7>{%
...
%
\only<-9>{\item<8-> Die Funktion \pythonil{int(a)} wandelt einen Wert~\pythonil{a} eines anderen Datentypen in einen \pythonil{int}\nobreakdashes-Wert um. \pythonil{a}~könnte \DEzB\ ein String sein.\uncover<9->{ \pythonil{int(\"1111\")} ergibt daher den Integer~\pythonil{1111}.%
}}%
%
...
%
\only<-15>{\item<14-> Die Funktion~\pythonil{float} wandelt einen Wert~\pythonil{a} eines anderen Datentypen in einen \pythonil{float}\nobreakdashes-Wert um. \pythonil{a}~könnte \DEzB\ ein String sein.\uncover<15->{ \pythonil{float} erkennt die wissenschaftliche Notation und \pythonil{float(\"2.233e4\")} wird daher~\pythonil{22330.0}.%
}}%
%
...
%
\only<-23>{\item<22-> Die Funktion~\pythonil{bool} wandelt einen Wert~\pythonil{a} eines anderen Datentypen in einen \pythonil{bool}\nobreakdashes-Wert um. \pythonil{a}~könnte \DEzB\ ein String sein.\uncover<23->{ Aus \pythonil{bool(\"True\")} wird daher~\pythonil{True}.%
}}%
%
\only<-29>{\item<24-> Die Funktion~\pythonil{bool} wandelt einen Wert~\pythonil{a} eines anderen Datentypen in einen \pythonil{bool}\nobreakdashes-Wert um. \pythonil{a}~könnte \DEzB\ ein String sein.\only<-27>{\uncover<25->{ Und \pythonil{bool(\"False\")} wird zu~\pythonil{False}.\uncover<26->{ %
\alert{Halt!}\uncover<27->{ \alert{Nein wird es nicht!}}}}}\uncover<28->{ \alert{Und \pythonil{bool(\"False\")} wird auch zu~\pythonil{True}!}%
}}%
%
\only<-30>{%
\item<29-> Der Grund ist, dass die Funktion \pythonil{bool} in so genanntes \alert{truth value testing} durchführt\cite{PSF:P3D:TPSL:BIF:B,PSF:P3D:TPSL:TVT}.%
}%
%
\only<-32>{%
\item<30-> Objekte bekommen dabei den Wert \pythonil{True}, \alert{\textbf{es sei denn}}, sie bieten selber eine explizite Umrechnung in den Type \pythonil{bool} an (und die ergibt \pythonil{False}) oder sie haben eine Länge (und die ist~0).%
}
%
\only<-32>{%
\item<31-> \pythonil{\"False\"} ist ein String mit einer Länge größer als~0.\uncover<32->{ Daher ist \pythonil{bool(\"False\")} \pythonil{True}.}%
}%
%
\only<-34>{%
\item<33-> Welchen Wahrheitswert hat dann der leere String \pythonil{\"\"}?\uncover<34->{ Der wird dann wirklich zu \pythonil{False}.}%
}%
%
\only<-36>{%
\item<35-> Für einen beliebigen nicht-leeren string, \DEzB\ \pythonil{\"blabla\"}\only<-35>{\dots}\uncover<36->{ gibt \pythonil{bool} uns aber immer \pythonil{True}.}%
}%
%
\item<37-> Von der Überraschung mit \pythonil{bool} abgesehen war das relativ einfach.%
%
\end{itemize}%
%
...
\locateGraphic{25-32}{width=0.8\paperwidth}{graphics/fromStr/fromStr18boolFalseDone}{0.1}{0.37}%
\locateGraphic{33}{width=0.8\paperwidth}{graphics/fromStr/fromStr19boolEmptyStr}{0.1}{0.37}%
\locateGraphic{34}{width=0.8\paperwidth}{graphics/fromStr/fromStr20boolEmptyStrDone}{0.1}{0.37}%
\locateGraphic{35}{width=0.8\paperwidth}{graphics/fromStr/fromStr21boolBlabla}{0.1}{0.37}%
\locateGraphic{36}{width=0.8\paperwidth}{graphics/fromStr/fromStr22boolBlablaDone}{0.1}{0.37}%
\end{frame}%
%
%
...
%
\only<-29>{\item<26-> Wollen wir einen String über mehrere Zeilen hinweg schreiben, dann können wir ans Ende der Zeile einfach ein Backslash~(\inQuotes{\textbackslash}) schreiben.\uncover<27->{ Wenn wir dann \keys{\return}~drücken, geht es in der nächsten Zeile einfach weiter, die im Interpreter dann mit drei Punkten beginnt.\uncover<28->{ So können wir den Text weiterschreiben.\uncover<29->{ Der String beinhaltet dann keinen Zeilenumbruch.}}%
}}%
%
...
\item<4-> Danach geben wir die Ergebnisse unserer Berechnungen wieder als Text aus. Dabei können wir \DEzB\ f\nobreakdashes-Strings nutzen, um andere Datentypen elegant als Strings zu formatieren.%
\item<5-> Manchmal wollen wir auch mit Strings direkt arbeiten, \DEzB\ Teile von ihnen extrahieren oder Groß-\ in Kleinbuchstaben umwandeln.%
