...
%
\begin{itemize}%
\item Tupels sind Listen sehr ähnlich, mit drei wichtigen Unterschieden\only<-1>{.}\uncover<2->{:%
%
\begin{enumerate}%
%
\item Tupels sind unveränderlich.\uncover<3->{%
...
Tupels sind von Anfang auch dazu gedacht, Objekte verschiedenen Typs zu beinhalten.\uncover<8->{ %
...
%
\only<-7>{%
\item<3-> Wir können so Tupels mit beliebig vielen typisierten Elementen deklarieren.%
}%
%
%\only<-7>{%
...
%
\item<6-> Tupels können genau wie Listen und Strings indiziert und gesliced werden.%
%
...
%
\begin{frame}[t]%
\frametitle{Tupels mit Elementen verschiedener Typen}%
%
\parbox{0.4\paperwidth}{\small{%%
\begin{itemize}%
%
\only<-5>{%
\item Wir können auch Tupels mit Elementen verschiedener Typen erstellen.%
}%
%
\only<-6>{%
...
%
\only<-7>{%
\item<3-> Tupels (und Listen) unterstützen alle sechs Vergleichsoperationen \pythonilIdx{<}, \pythonilIdx{<=}, \pythonilIdx{==}, \pythonilIdx{>=}, \pythonilIdx{>}, und~\pythonil{!=}.%
}%
%
\only<-8>{%
\item<4-> Natürlich können wir Tupels auch in Listen oder Tupels reintun.%
}%
%
\only<-9>{%
\item<5-> Der \glslink{typeHint}{Type-Hint} \pythonil{list[tuple[str, int, float]]} definiert eine Liste, die Tupels es oben genannten Typs beinhalted.%
}%
%
\only<-10>{%
...
%
\only<-13>{%
\item<7-> Angenommen, wir vergleichen wir zwei Tupels~\pythonil{x} und~\pythonil{y}.\uncover<8->{ %
...
Wenn das erste Element von~\pythonil{x} gleich dem erste Element von~\pythonil{y} ist, dann geht der Vergleich mit den zweiten Elementen weiter, wenn beide Tupels mindestens 2~Elemente haben~(andernfalls ist das kürzere Tupel kleiner).\uncover<11->{ %
Und so weiter.}}}}%
}%
%
\item<12-> Deshalb können wir Listen von Tupels sortieren.%
%
\item<13-> Genau wie Listen können wir Tupels auf mehrere Variablen auspacken.%
%
\item<14-> In Fällen, wo es keine Verwechslung geben kann, können wir Tupels sogar ohne die runden Klammern definieren.%
%
\end{itemize}%
}}%
%
...
%
\only<-4,9->{%
\item Prüfen wir nun, ob Tupels wirklich unveränderlich sind.%
}%
%
...
Packen Sie immer nur unveränderliche Ojekte in Tupels.\uncover<6->{ %
Veränderliche Objekte in Tupels können immer noch verändert werden, wodurch die Tupels dann keine Konstanten mehr sind.\uncover<7->{ %
Programmierer nehmen aber an, dass Tupels Konstanten sind.\uncover<8->{ %
