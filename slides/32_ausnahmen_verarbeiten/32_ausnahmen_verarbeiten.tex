...
%
\only<-3>{%
\item<2-> Wir schreiben \pythonil{try:} und danach platzieren wir den Kode, der eine Ausnahme auslösen kann, in einen eingerückten Block.%
}%
%
\only<-3>{%
...
\begin{itemize}%
\only<-10>{%
\item Wir können nun alle möglichen \pythonilsIdx{Exception} abfangen.%
}%
%
\only<-11>{%
...
%
\only<-13>{%
\item<4-> Es ist akzeptabel, dass das mit einem System-bezogenen Fehler schief geht.%
%
...
%
\only<-18>{%
\item<14-> Um \pythonil{sqrt(1 / 0)} aufzurufen, muss der Interpreter zuerst \pythonil{1 / 0} berechnen.%
}%
%
\only<-19>{%
...
Beachten Sie dass, wenn Sie auf Variablen zugreifen, die in einem \pythonil{try}-block zugewiesen werden.}}}%
}%
}%
\end{frame}%
%
...
%
\only<-10>{%
\item<6-> Der \pythonilIdx{except}-Block wird dadurch sofort abgebrochen und erklärender Text wird auf \glslink{stderr}{stderr} ausgegeben.%
}%
%
...
%
\item<11-> Daher ist garantiert, dass die Variablen existieren und dass ihnen Werte zugewiesen wurden.%
%
...
%
\only<-9>{%
\item Wir wissen, dass Fehler in Prozessen auftauchen können.%
}%
%
\only<-10>{%
...
%
\item<12-> Wenn wir aber abbrechen, ohne die Datei vorher zu schließen, dann könnte der gesamte Inhalt der Datei verloren gehen.%
%
...
%
\only<-6>{%
\item<3-> In einem \pythonilIdx{try}-Block versucht die Funktion \pythonil{a} durch \pythonil{b} zu teilen und das Ergebnis mit Hilfe eines \glslink{fstring}{f-Strings} via \pythonil{print} auszugeben.%
}%
%
\only<-7>{%
...
%
\only<-14>{%
\item<10-> Dann brauchen wir den Datentyp auch nicht importieren.%
}%
%
\only<-15>{%
...
%
\item<5-> Wir können klar ausdrücken, welche Ausnahmen unser Kode auslöst, \DEzB\ in \glslink{docstring}{Docstrings}.%
%
