...
%
\begin{itemize}%
\item Tupel~\inEN{tuples} sind Listen sehr ähnlich, mit drei wichtigen Unterschieden\only<-1>{.}\uncover<2->{:%
%
\begin{enumerate}%
%
...
Wenn Sie nicht vor haben, die Sequenz zu verändern, benutzen Sie ein Tupel~(\pythonil{tuple}).}%
}%
\end{frame}%
%
\section{Beispiele}%
...
Wenn Sie wirklich unveränderliche Datenstrukturen brauchen, dann packen Sie immer nur unveränderliche Objekte in Tupel.\uncover<6->{ %
...
Programmierer nehmen aber oft an, dass Tupel Konstanten sind.\uncover<8->{ %
