...
%
\only<-11>{%
\item<7-> Die Funktion \pythonil{factorial} und eine neue Funktion namens \pythonil{sqrt}.%
}%
%
\only<-12>{%
\item<8-> Die \pythonil{sqrt}-Funktion basically ist im Grunde unser Kode von Einheit~\unitSqrtHeron\ mit unserer Implementierung von Heron's Methode zur Berechnung der Quadratwurzel.%
}%
%
\only<-13>{%
...
%
\only<-14>{%
\item<10-> Unser neues Modul hat den Namen \textil{my_math}, weil es in der Datei \programUrl{functions:my_math} gespeichert ist.%
}%
%
\only<-15>{%
