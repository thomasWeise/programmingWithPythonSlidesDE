...
\only<-9>{%
\only<-8>{%
\item<3-> \emph{Schleifen}~\inEN{loops}~erlauben uns genau das, nämlich eine Aktion mehrfach auszuführen.%
}%
%
\only<-8>{%
...
Das \emph{Structured Program Theorem} sagt aus, dass jede berechenbare Funktion mit nur drei verschiedenen Kontrollflusselementen berechnet werden kann, nämlich mit \emph{(1)}~dem sequenziellen Ausführen von Befehlen, \emph{(2)}~dem selektiven Ausführen von Befehlen~(also mit Alternativen), und~\emph{(3)}~dem iterativen~(repetitiven) ausführen von Befehlen~(bis eine bestimmte Bedingung eintrifft)\cite{B1964OAFOTMATRPL,BJ1966FDTMALWOTFR,S2011C1GIICS:TBJTAAITSPWP,H1980OFT}.%
\end{definition*}%
%%
\uncover<10->{%
\begin{itemize}%
\only<-12>{%
\item Wir kennen bereits die ersten beiden Kontrollflusselemente.%
}%
\item<11-> Und jetzt lernen Sie das dritte Element, nämlich Schleifen.%
\item<12-> Danach sind wir im Prinzip in der Lage, \emph{jede} Berechnung durchzuführen, die mit unseren Computern möglich ist.%
\item<13-> Alles was dann folgt sind nur Konzepte, die dafür da sind, unser Leben als Programmierer leichter zu machen.%
\end{itemize}}}%
\end{frame}%%
%
...
\begin{itemize}%
\only<-6>{%
\item Die Syntax einer \texttt{for}-Schleife ist einfach\cite{PSF:P3D:TPT:MCFT}.%
}\only<-6>{%
...
%
\item<2-> In ihrer einfachsten Form wird eine \pythonil{for}-Schleife aber auf eine \pythonilIdx{range} von Ganzzahlen angewandt\cite{PSF:P3D:TPT:MCFT}.%
%
...
%
\item<5-> \pythonil{range(6, 9)} gibt uns eine Sequenz von Ganzzahlen, die mit~6 anfängt, in Schritten von~1 ansteigt, und \emph{vor}~9 aufhört, also im Grunde~\intRange{6}{8}.%
%
...
Wenn der Wert einer Variable oder eines Parameters egal ist, dann sollten wie diese~\pythonil{_}~nennen\cite{PEP635}. %
\uncover<20->{%
...
%
\item<4-> Für den ersten Fall gibt es das Schlüsselwort~\pythonilIdx{continue}, für den zweiten Fall das Schlüsselwort \pythonilIdx{break}\cite{PSF:P3D:TPT:MCFT}.%
\end{itemize}%
\end{frame}%
%
\begin{frame}[t]%
...
%
\item<23-> Mit \pythonilIdx{break} und \pythonilIdx{continue} haben wir nun zwei neue Werkzeuge, mit denen wir entweder die die ganze Schleife abbrechen können oder die aktuelle Iteration beenden~(und mit der nächsten weitermachen können, wenn es eine gibt).%
\end{itemize}%
}}%
%
...
\begin{itemize}%
%
\item Genau wir Alternativen, können wir Schleifen beliebig verschachteln~\inEN{nest}.%
%
...
%
\only<-34>{%
\item<31-> Für alle \pythonil{candidate <= 8} wird diese Schleife gar nicht ausgeführt, denn \pythonil{isqrt(8) == 2}.%
}%
%
\only<-35>{%
...
%
\only<-36>{%
\item<33-> Für \pythonil{candidate > 8}, geht \pythonil{check} dann von~\pythonil{3} bis$\left\lfloor\sqrt{\pythonil{candidate}}\right\rfloor$.%
}%
%
\only<-37>{%
...
\begin{itemize}%
\item Wir haben bei der Einführung der \pythonil{for}-Schleife gesagt, dass sie über eine \emph{Sequenz} von Daten iteriert.%
%
...
\only<-9>{%
\item<6-> Im Körper~\inEN{body} der Schleife rufen wir dann \pythonil{txt.append(f\"\{i = \}\")} auf.
}%
\only<-10>{%
...
\only<-15>{%
\item<12-> Wir verwandeln sie wieder via einem \glslink{fstring}{f-String} \pythonil{f\"\{f = \}\"} in Text um an hängen diesen via \pythonil{append} an die Liste \pythonil{txt} an.%
}%
\only<-16>{%
...
%
\item<51-> Daher produziert \pythonil{\", \".join(txt)} als Ergebnis \textil{i = 1, i = 2, i = 3, i = 50,} \textil{f = 7.6, ...}.%
%
...
\only<-5>{%
\item<3-> Als wir mit dem Originalprogramm angefangen haben, dann haben wir sofort die Zahl~\pythonil{2} als eine Primzahl in unsere Liste~\pythonil{primes} gespeichert.%
}%
\only<-6>{%
\item<4-> Wir haben dann alle ungeraden Zahlen kleiner als 200 als potentielle Prinzahlen \pythonils{candidate} durchprobiert.%
}%
\only<-8>{%
...
%
\item<3-> Wir können mit \pythonil{for}-Schleifen über Ganzzahl-Sequenzen~(\pythonilIdx{range}) iterieren oder über die Daten in einer Kollektion.%
%
