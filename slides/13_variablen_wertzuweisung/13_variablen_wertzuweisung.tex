...
\item<6-> Dafür gibt es \alert{Variablen}.
\end{itemize}%
\end{frame}%
%
...
\item<5-> Wir üben das von Anfang an. Wir \alert{müssen} das konsequent durchziehen.%
\end{itemize}%
\uncover<6->{%
...
\begin{itemize}%
\item<7-> In diesem Kurs werden wir immer grundlegende neue Elemente der Programmiersprache und wichtige Best Practices zusammen lernen.%
\end{itemize}%
}%
\end{frame}%
%
...
\item<5-> Weil das Befolgen von \emph{Best Practices} nichts ist, dass man nachträglich später machen kann.%
\item<6-> Sie werden \alert{niemals} die Zeit haben, den Stil Ihres alten Kodes zu verbessern.%
\item<7-> Das ist auch nichts, dass man einfach so anfangen kann zu machen.%
\item<8-> Wenn Sie gelernt haben, eine Sache auf eine bestimmte Art zu machen, dann ist es immer schwer, auf eine andere Art umzuschalten.%
\item<9-> Wenn ein Koch-Azubi nicht beigebracht bekommt, sich vor dem Essenmachen die Hände zu waschen, dann wird er es später auch nicht von sich aus konsistent machen, auch dann nicht, wenn es ihm einmal explizit gesagt wird.%
\item<10-> Das befolgen von Stilrichtlinien und Best Practices ist eine Angewohnheit.%
\item<11-> Und die lernen wir hier gleich mit.%
\end{itemize}%
\end{frame}%
%
\begin{frame}%
...
%
\only<-24>{%
\item<22-> Wenn wir diese Ecken mit dem Zentrum des Kreises verbinden, dann zerschneiden die neue Linien die Seiten des Sechsecks genau in jeweils zwei gleich große Hälften und sie tun das in einem~$90^{\circ}$~Winkel.%
}%
%
\only<-26>{%
...
\listingPython{6-11}{variables:pi_liu_hui}{0.025}{0.45}{0.95}{1.25}%
