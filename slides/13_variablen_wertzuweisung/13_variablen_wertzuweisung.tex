...
%
\only<-33>{%
\item<32-> Zu guter Letzt drucken wir noch \pythonil{print(f\"\{new_var = \}.\" )}, was als \textil{new_var = 14.0.} auf \pgls{stdout} erscheint.%
}%
%
\only<-34>{%
\item<33-> Schauen wir uns die gesamte Ausgabe auf den \pgls{stdout} unseres Programms an.%
\item<34-> Das passt.%
}%
%
\only<-36>{%
\item<35-> Nun führen wir das Program im \glslink{terminal}{Terminal} aus.%
}%
\only<-37>{%
\item<36-> Unter \ubuntu\ \linux\ können wir ein Terminal durch Druck auf \ubuntuTerminal\ öffnen, unter \microsoftWindows\ durch \windowsTerminal.%
}%
\only<-38>{%
\item<37-> Auf beiden \glslink{OS}{Betriebssystemen} gehen wir mit Hilfe des Kommandos~\bashil{cd} in den Ordner mit der Programmdatei \programUrl{variables:assignment}.%
}%
\only<-39>{%
\item<38-> Dort können wir diese dann durch \bashil{python3 assignment.py} ausführen.%
\item<39-> Die Ausgabe ist natürlich die selbe.%
}%
%
\only<-40>{%
\item<40-> Alternativ können wir die Programmdatei auch in \pycharm\ dem \glsFull{ide} öffnen.%
}%
%
\only<-41>{%
\item<41-> Um sie auszuführen, können wir auf den Dateinamen im Project-View rechts-klicken und dann auf \menu{Run `assignment'}.%
}%
%
\only<-42>{%
\item<42-> Oder wir drücken einfach \keys{\ctrl+\shift+F10} im Editor.%
}%
%
%\only<-43>{%
\item<43-> So oder so wird das Programm ausgeführt und wir bekommen wieder genau die erwartete Ausgabe.%
%}%
\end{itemize}%
%
\listingPython{2-8,10-20,22-26,28-30,32}{variables:assignment}{0.15}{0.38}{0.7}{0.7}%
\listingOutput{33-34}{variables:assignment}{}{0.05}{0.45}{0.9}{0.7}%
%
