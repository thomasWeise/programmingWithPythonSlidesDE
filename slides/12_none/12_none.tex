...
%
\only<-6>{\item<5-> Normalerweise, wenn wir einen Wert~(\DEzB~\pythonil{3}) in die \python-Konsole schreiben und~\keys{\return} drücken, dann wird der Wert uns wieder ausgegeben. Schreiben wir dagegen \pythonil{None} in die \python-Konsole und drücken~\keys{\return}, dann\only<-5>{\dots}\uncover<6->{ %
passiert gar nichts.%
}}%
%
...
\item<30-> Wenn wir prüfen wollen, ob \pythonil{X} \pythonil{None} ist, dann schreiben wir \pythonil{x is None}.%
}%
%
\end{itemize}%
%
